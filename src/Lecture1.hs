...
lowerAndGreater n list = show n ++ " is greater than " ++ show (greaters n list) ++ " elements and lower than " ++ show (lowers n list) ++ " elements"

greaters :: Int -> [Int] -> Int
greaters _ [] = 0
greaters n (x : xs)
  | n > x = 1 + greaters n xs
  | otherwise = greaters n xs

lowers :: Int -> [Int] -> Int
lowers _ [] = 0
lowers n (x : xs)
  | n < x = 1 + lowers n xs
  | otherwise = lowers n xs
